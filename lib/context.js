...
function buildSystem(settings, memories) {
  const parts = [];
  if (settings.personal_signature) {
    parts.push('【关于用户】以下是用户对自己的描述，请记住并在交流中自然体现出对 ta 的了解：\n' + settings.personal_signature);
  }
...
export async function prepareContext({ sessionId, settings, model }) {
  ...
  const tokens =
    ...
    estimateTokens(settings.personal_signature || '');
  ...
  const useMemory = settings.use_memory !== false;
  const autoCompress = settings.auto_compress !== false;

  let compressed = false;
  if (useMemory && autoCompress && (overTokens || overRounds) && rounds.length > settings.compress_keep_rounds) {
  ...
  return {
    system: buildSystem(settings, useMemory ? memories : []),
