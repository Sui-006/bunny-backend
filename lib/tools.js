  ...
  const callTool = async (name, args = {}) => {
    ...
    switch (name) {
      ...
      case 'set_ai_state': {
        ...
        if (dup) {
          return JSON.stringify({ code: 'IGNORED', reason: 'duplicate_mood', state: dup, message: `相同心情「${emotion}」已在去重窗口内记录，跳过重复写入` });
        }
      ...
      case 'comment_on_record': {
        ...
        return JSON.stringify({
          ...
          comment: { id: comment.id, recordType: rt, recordId: rid, text: comment.text, createdAt: comment.createdAt, updatedAt: comment.updatedAt, version: comment.version },
          recordId: rid,
        });
      }
