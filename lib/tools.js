...
const TOOL_DEFS = [
  ...
  S('list_shopping', '列出购物清单条目（缺省只列未购；可按 listId=自己/家人 过滤）', { listId: { type: 'string' }, includeCompleted: { type: 'boolean' } }),
  ...
  // ---- 编辑：购物 ----
  S('add_shopping_item', '添加购物条目（listId=自己 或 家人）', { title: { type: 'string' }, listId: { type: 'string' }, note: { type: 'string' }, category: { type: 'string' }, priority: { type: 'string' }, reason: { type: 'string' } }, ['title']),
  S('update_shopping_item', '修改购物条目', { id: { type: 'string' }, title: { type: 'string' }, note: { type: 'string' }, category: { type: 'string' }, priority: { type: 'string' }, reason: { type: 'string' } }, ['id']),
  ...
  // ---- 编辑：AI 动态 ----
  S('create_ai_activity', '写入一条 AI 动态（记录 AI 自己做过/想到/值得记住的事）。content=动态正文（第一人称、简洁，一句话即可）；type 缺省 chat。当用户分享了值得记录的事——如完成一件事、有情绪波动、做了决定、有进展或里程碑——你可以自主写一条；但普通寒暄、闲聊、提问、无实质内容的对话不要写，避免刷屏。', {
  ...
  const callTool = async (name, args = {}) => {
    ...
    switch (name) {
      ...
      case 'list_shopping': {
        ...
        if (args.listId) list = list.filter((it) => it.listId === args.listId);
      ...
      case 'add_shopping_item': {
        if (!args.title) return JSON.stringify({ code: 'FAILED', error: '缺少 title' });
        const item = {
          id: randomUUID(), listId: args.listId || 'mine', title: String(args.title), note: args.note || '', category: args.category || '其他',
      ...
      case 'update_shopping_item': {
        ...
        for (const k of ['title', 'note', 'category', 'priority', 'listId']) if (args[k] != null) it[k] = String(args[k]);
      ...
      case 'create_ai_activity': {
        ...
        const activity = { id: randomUUID(), type, text, time: Date.now() };
