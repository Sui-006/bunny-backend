...
import {
  ...
  appendAiActivity, isAiActivity, aiActivities, appendAiState,
} from './domain.js';
...
import { songDetail, hotComments, lyric, authState, search } from './netease.js';
...
const TOOL_DEFS = [
  ...
  S('search_song', '搜索网易云歌曲（按歌名/歌手/关键词），返回候选曲目的 id/歌名/歌手/专辑/封面/时长。用于「播放 XX 的歌」「找 XX 的歌」：先搜索拿 id，再 queue_song 加入播放列表，最后 play_song 播放。', {
    query: { type: 'string', description: '搜索关键词（歌名、歌手或组合）' },
    limit: { type: 'integer', description: '返回数量，缺省 5，最大 10' },
  }, ['query']),
...
export const TOOL_DOMAIN_MAP = {
  ...
  get_recent_music: ['music'], search_song: ['music'], get_listening_context: ['music'], start_listening_session: ['music'], end_listening_session: ['music'],
  ...
  const callTool = async (name, args = {}) => {
    ...
    switch (name) {
      ...
      case 'search_song': {
        ...
        if (!q) return JSON.stringify({ code: 'FAILED', error: '缺少搜索关键词' });
        const limit = Math.max(1, Math.min(10, parseInt(args.limit) || 5));
        const songs = await search(q, limit);
        return JSON.stringify({ code: 'OK', songs });
      }
