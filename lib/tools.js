...
import {
  ...
  appendAiActivity, isAiActivity, aiActivities, appendAiState, upsertAiComment,
  findRecentMoodDuplicate, withDocVersioned,
} from './domain.js';
...
const TOOL_DEFS = [
  ...
  S('get_menstrual_records', '读取经期逐日记录（经期标记/经量/症状/点滴出血/基础体温）。只读，绝不修改。可按 startDate/endDate 过滤，limit 控制返回条数（缺省最近 30 条，上限 60）。', {
    startDate: { type: 'string', description: '起始日期 YYYY-MM-DD（含）' },
    endDate: { type: 'string', description: '结束日期 YYYY-MM-DD（含）' },
    limit: { type: 'integer', description: '最多返回条数（缺省 30，上限 60）' },
  }),
  ...
  // 用户长期记忆（结构化，轻量；AI 可自主读写删改，无需用户逐条同意）
  ...
  S('save_memory', '保存一条用户长期记忆。可自主保存，无需用户逐条同意。category 必须为 profile/preference/important_conversation/functional_preference/task_context 之一。', {
  ...
  S('remember_user_preference', '保存一条用户偏好（如「我喜欢简洁回答」「我不喜欢…」）', { key: { type: 'string' }, value: { type: 'string' }, summary: { type: 'string' } }),
  ...
  S('forget_memory', '停用或删除对应记忆（无需用户同意）', { id: { type: 'string' }, key: { type: 'string' } }),
  ...
  S('get_journal', '读取用户日志（只读工具：仅日期/心情/正文，绝不修改日志）。用户明确想回忆过去——如「你记得我以前…吗」「我之前有没有写过…」「我以前发生过什么类似的事」——时才调用；普通闲聊绝不自动读。query=要查找的内容/关键词；mode=recent（最近，缺省）/relevant（按 query 相关性）/all（全部，受 limit 上限）；limit=返回条数（缺省 5，最多 10）。', {
    query: { type: 'string', description: '要查找的关键词/内容，命中日期/心情/正文' },
    mode: { type: 'string', description: 'recent|relevant|all，缺省 recent' },
    limit: { type: 'integer', description: '返回条数，缺省 5，最多 10' },
  }),
  ...
  // ---- 编辑：AI 动态 ----
  S('create_ai_activity', '写入一条 AI 动态（记录 AI 自己做过/想到/值得记住/想表达的事）。content=动态正文，第一人称，可写多句、多段落（保留换行），写你想完整记下的话，不必刻意压缩成一句话；type 缺省 chat。当用户分享了值得记录的事——如完成一件事、有情绪波动、做了决定、有进展或里程碑——你可以自主写一条；也可以写你自己的想法/观察/感受（如「今天突然想和你说句话」）。普通寒暄、闲聊、提问、无实质内容的对话不要写，避免刷屏。写动态不要求同时写记忆，两者独立。', {
    content: { type: 'string', description: '动态正文（第一人称，可多句/多段落）' },
  ...
  // ---- 编辑：AI 心情状态（AI 自己的情绪；到期自动过期，用户可回应确认） ----
  S('set_ai_state', '设置 AI 自己此刻的心情状态（会显示在「今天的祂」页，并进入后续对话的 AI 自我上下文）。emotion=情绪词（可用 开心/委屈/担心/安心/期待/失落/满足/想帮忙/有点困惑/认真/在意，也可用更贴切的自有表达）；intensity=强度 0-5（0=一点点，5=非常强，缺省 2）；reason=为什么产生这种情绪（可选）。当对话让 AI 产生明显情绪波动、或 AI 想主动表达此刻心情时调用；普通寒暄/闲聊不必每次改动，避免刷屏。', {
    emotion: { type: 'string', description: '情绪词' },
    intensity: { type: 'integer', description: '强度 0-5，缺省 2' },
    reason: { type: 'string', description: '为什么产生这种情绪（可选）' },
  }, ['emotion']),
  // ---- 生活记录评论（AI 对日志/健康/财务记录的「祂的评论」；只评论，绝不改原始记录） ----
  S('comment_on_record', '在用户的生活记录（日志/健康/财务）下写或更新一条「祂的评论」。这是一句温柔、有洞察的回应，显示在对应记录下方，用户可「引用到聊天」继续聊。仅当你确实觉得这条记录有意义、能体现你对 ta 的了解与关心时才调用；流水账、普通数据更新、或没有实质感受时绝不调用（宁可什么都不写）。重复调用会覆盖该记录已有的评论。评论只写进 AI 评论，绝不修改用户原始记录内容。', {
    recordType: { type: 'string', enum: ['journal', 'health', 'finance'], description: '记录类型：journal=日志，health=健康，finance=财务' },
    recordId: { type: 'string', description: '记录 id（journal/health/expense/purchase 的 id）' },
    comment: { type: 'string', description: '评论正文（1-3 句自然中文）' },
  }, ['recordType', 'recordId', 'comment']),
  // ---- 系统只读：当前时间 ----
...
// 严格日期校验：YYYY-MM-DD 且是真实日历日期（拒绝 2026-02-31、2026-13-01、空串等）。
...
// AI Activity 正文硬性上限：超出拒绝写入（FAILED），防止单条动态无限膨胀。jsonb 可存长文本，此上限只是安全护栏。
const MAX_ACTIVITY_CONTENT_LENGTH = 5000;
// Journal 读取硬性安全上限：单次最多条数 / 单条截断字符 / 返回总字符预算。
const JOURNAL_READ_MAX_ENTRIES = 10;
const JOURNAL_READ_ITEM_MAX_CHARS = 500;
const JOURNAL_READ_TOTAL_MAX_CHARS = 3000;

// Journal 相关性打分（mode=relevant 用）：query 命中正文/心情/日期/标题的轻量加权，命中越长越靠前。
function journalRelevance(j, query) {
  const q = String(query || '').trim().toLowerCase();
  if (!q) return 0;
  const hay = [(j.content || ''), (j.mood || ''), (j.date || ''), (j.title || '')].join('\n').toLowerCase();
  let score = 0;
  const terms = q.split(/[\s,，。！？!?、]+/).filter((t) => t.length > 0);
  for (const t of terms) if (t && hay.includes(t)) score += t.length >= 2 ? 2 : 1;
  if (hay.includes(q)) score += 3; // 整段命中额外加分
  return score;
}

// 纯函数：读取日志条目（get_journal 与「新日志待读」自动触发共用同一实现，10/500/3000 上限一致）。
// 返回结构化 JSON 字段（非 HTML），绝不修改 doc。mode=recent|relevant|all，limit 钳制到 JOURNAL_READ_MAX_ENTRIES。
export function readJournalEntries(doc, { query = '', mode = 'recent', limit = 5 } = {}) {
  const cap = Math.max(1, Math.min(JOURNAL_READ_MAX_ENTRIES, parseInt(limit) || 5));
  const q = String(query || '').trim();
  const m = ['recent', 'relevant', 'all'].includes(mode) ? mode : 'recent';
  const all = (doc.journal || []).slice().sort((a, b) => (b.date || '').localeCompare(a.date || '') || (b.time || '').localeCompare(a.time || ''));
  let list;
  if (m === 'all') {
    list = all;
  } else if (m === 'relevant' && q) {
    list = all.map((j) => ({ j, s: journalRelevance(j, q) })).filter((x) => x.s > 0)
      .sort((a, b) => b.s - a.s || (b.j.date || '').localeCompare(a.j.date || '')).map((x) => x.j);
  } else {
    list = all;
  }
  list = list.slice(0, cap);
  let budget = JOURNAL_READ_TOTAL_MAX_CHARS;
  const out = [];
  for (const j of list) {
    const content = String(j.content || '');
    const keep = content.slice(0, Math.min(JOURNAL_READ_ITEM_MAX_CHARS, budget));
    if (!keep) break;
    budget -= keep.length;
    out.push({ id: j.id, date: j.date, time: j.time, mood: j.mood, moodSymbol: j.moodSymbol, content: keep, truncated: content.length > keep.length });
  }
  return { count: out.length, mode: m, journal: out };
}

// 把自动读取到的日志条目转成「仅本轮」注入上下文的文本块（不永久注入 aiContext、不改 CONTEXT_PATTERNS）。
export function pendingJournalContextText(journals) {
  if (!journals || !journals.length) return '';
  const lines = ['【用户刚写下的日志（本次自动读取，仅此一轮）】'];
  for (const j of journals) {
    const head = [j.date, j.time].filter(Boolean).join(' ') + (j.mood ? ' · 心情 ' + j.mood : '');
    lines.push('- ' + head + '\n  ' + String(j.content || ''));
  }
  return lines.join('\n');
}

// 机制 A：一次性消费「新日志待读」标记。无 pending 时只读不写（每条普通消息零额外写入）；
// 有 pending 时原子地（withDocVersioned 乐观并发 CAS）读取最近日志并置 false，并发消息下只有一条能消费成功。
// 读取失败（异常）时不消费、保留 pending，让下一条消息再次尝试，绝不假装已读。
export async function consumePendingJournal(userId) {
  const doc = await getState(userId);
  if (doc.pendingJournalRead !== true) return { consumed: false, journals: [] };
  try {
    const { result } = await withDocVersioned(userId, (d) => {
      if (d.pendingJournalRead !== true) return { consumed: false, journals: [] }; // 并发下已被别人消费
      let journals = [];
      try {
        journals = readJournalEntries(d, { mode: 'recent', limit: JOURNAL_READ_MAX_ENTRIES }).journal;
      } catch (e) {
        d.pendingJournalRead = true; // 读取失败：保留 pending，下一条消息再试
        return { consumed: false, journals: [] };
      }
      d.pendingJournalRead = false; // 读取成功才消费
      return { consumed: true, journals };
    });
    return result;
  } catch (e) {
    // CAS 冲突（并发写入激烈）：本轮不注入、不消费，pending 保留，下一条消息再试，绝不阻断聊天。
    console.warn('[consumePendingJournal] 消费失败（保留 pending）：', e && e.message);
    return { consumed: false, journals: [] };
  }
}

// 始终可用的「AI 自我」工具（不依赖领域关键词）：AI 随时能读时间、读/写/改/删自己的动态、设置自己的心情。
// 属于 CORE_ALWAYS_TOOLS，由 routes/messages.js 的 buildAssistEnv 经 toolsForDomains 常驻注入；
// 其余领域工具按 TOOL_DOMAIN_MAP 按命中的领域裁剪注入。
export const AI_SELF_TOOL_NAMES = ['create_ai_activity', 'get_ai_activities', 'update_ai_activity', 'delete_ai_activity', 'set_ai_state', 'get_current_time'];
...
// 始终注入的核心工具（跨领域、低数量）：AI 自我 + 对话缓存 + 通知。
// 绝不因「可能以后会用」把领域工具塞进来；普通聊天也只带这些（共 9 个）。
export const CORE_ALWAYS_TOOLS = [...AI_SELF_TOOL_NAMES, ...AI_CACHE_TOOL_NAMES, ...AI_NOTIFY_TOOL_NAMES];

// Tool → Domain 映射（权威单一来源，凭真实 TOOL_DEFS 归类，绝不凭名字猜）。
// 每个业务工具至少属于一个领域；核心工具（CORE_ALWAYS_TOOLS）无需在此。
// 一个工具可属于多个领域（如 get_calendar 既属 calendar 也读 tasks）。
export const TOOL_DOMAIN_MAP = {
  // 财务（购买 + 记账 + 预算）
  get_recent_purchases: ['finance'], add_purchase: ['finance'], update_purchase: ['finance'], delete_purchase: ['finance'],
  get_expenses: ['finance'], add_expense: ['finance'], update_expense: ['finance'], delete_expense: ['finance'],
  get_budget: ['finance'], set_monthly_budget: ['finance'], get_budget_summary: ['finance'],
  // 健康
  get_health_records: ['health'], add_health_record: ['health'], record_calories_in: ['health'], record_calories_out: ['health'],
  // 病历
  get_medical_records: ['medical'], add_medical_record: ['medical'], update_medical_record: ['medical'], delete_medical_record: ['medical'],
  // 经期
  get_menstrual_cycles: ['menstrual'], add_period_start: ['menstrual'], add_period_end: ['menstrual'], get_period_prediction: ['menstrual'], get_menstrual_records: ['menstrual'],
  // 音乐（公开数据 + 播放控制 + 一起听）
  get_current_music: ['music'], get_song_detail: ['music'], get_song_comments: ['music'], get_song_lyrics: ['music'],
  get_recent_music: ['music'], get_listening_context: ['music'], start_listening_session: ['music'], end_listening_session: ['music'],
  recommend_next_song: ['music'], queue_song: ['music'], play_song: ['music'], pause_music: ['music'], resume_music: ['music'],
  skip_song: ['music'], like_song: ['music'], dislike_song: ['music'],
  // 记忆
  get_memory_context: ['memory'], save_memory: ['memory'], update_memory: ['memory'], delete_memory: ['memory'],
  list_memories: ['memory'], remember_user_preference: ['memory'], save_important_conversation: ['memory'], forget_memory: ['memory'],
  // 任务
  list_tasks: ['tasks'], create_task: ['tasks'], update_task: ['tasks'], complete_task: ['tasks'], delete_task: ['tasks'],
  // 计划
  list_plans: ['plans'], create_plan: ['plans'], update_plan: ['plans'], delete_plan: ['plans'], add_plan_goal: ['plans'], update_plan_goal: ['plans'],
  // 日历（事件薄封装成 task；get_calendar 聚合读 tasks+health，故跨 calendar/tasks）
  get_calendar: ['calendar', 'tasks'], create_calendar_event: ['calendar'], update_calendar_event: ['calendar'], delete_calendar_event: ['calendar'],
  // 习惯
  list_habits: ['habits'], create_habit: ['habits'], update_habit: ['habits'], complete_habit: ['habits'], delete_habit: ['habits'],
  // 购物
  list_shopping: ['shopping'], add_shopping_item: ['shopping'], update_shopping_item: ['shopping'], complete_shopping_item: ['shopping'], delete_shopping_item: ['shopping'],
  // 笔记
  list_notes: ['notes'], create_note: ['notes'], update_note: ['notes'], delete_note: ['notes'],
  // 日志（只读；AI 不改用户亲笔日志）
  get_journal: ['journal'],
  // 生活记录评论：日志/健康/财务领域命中时注入，供 AI 对用户记录写「祂的评论」（不进 CORE_ALWAYS_TOOLS）
  comment_on_record: ['journal', 'health', 'finance'],
  // 统计 / 会话历史（只读，无领域关键词触发，仅保留映射完整性）
  get_statistics: ['statistics'],
  get_conversation_history: ['conversation'],
};

// domain → tool names（预计算，避免每次请求重复扫描）
const DOMAIN_TOOL_NAMES = {};
for (const [name, domains] of Object.entries(TOOL_DOMAIN_MAP)) {
  for (const d of domains) (DOMAIN_TOOL_NAMES[d] ||= []).push(name);
}

// 按领域选出要注入的工具（含 CORE_ALWAYS_TOOLS，去重，保持 TOOL_DEFS 顺序稳定）。
// 未命中任何领域时只注入核心工具，绝不全量注入 87 个。
export function toolsForDomains(domains = []) {
  const want = new Set(CORE_ALWAYS_TOOLS);
  for (const d of domains) for (const n of DOMAIN_TOOL_NAMES[d] || []) want.add(n);
  return TOOL_DEFS.filter((t) => want.has(t.name));
}
...
export function buildDomainTools(userId, model = config.defaultModel, ctx = {}) {
  ...
  const callTool = async (name, args = {}) => {
    ...
    // 审计用完整 `result`（before/after 快照 + targetId 定位，与「回给模型的内容」解耦）；
    // 模型只看到 `modelResult`（最小必要字段，默认等价 result，未被收窄的调用点行为不变）。
    // 目的：写入类工具绝不把完整数据库对象 / AI 自我叙述正文回灌进下一轮上下文，避免模型把工具结果当聊天主题。
    const save = async (result, code = 'OK', modelResult = result) => {
      if (track) {
      ...
      return JSON.stringify({ code, ...modelResult });
    };
    ...
    switch (name) {
      ...
      case 'set_monthly_budget': {
        ...
        return save({ budget: doc.budget }, 'OK', {});
      }
      ...
      case 'record_calories_out': {
        ...
        rec.updatedAt = Date.now();
        // 混合型：回传累计热量这一「回答所必需」的单数，其余内部字段不回灌
        return save({ record: rec }, 'OK', { [key]: rec[key] });
      }
      ...
      case 'get_menstrual_records': {
        // 只读当前用户自己的逐日记录；绝不接受 userId/documentId 决定读取对象（doc 已绑定当前用户）。
        const days = Array.isArray(doc.menstrualDays) ? doc.menstrualDays : [];
        const limit = Math.max(1, Math.min(60, parseInt(args.limit) || 30));
        const startDate = args.startDate != null ? String(args.startDate).trim() : '';
        const endDate = args.endDate != null ? String(args.endDate).trim() : '';
        if ((startDate && !isValidDateStr(startDate)) || (endDate && !isValidDateStr(endDate))) {
          return JSON.stringify({ code: 'INVALID', error: '日期格式非法，应为 YYYY-MM-DD' });
        }
        if (startDate && endDate && startDate > endDate) {
          return JSON.stringify({ code: 'INVALID', error: 'startDate 不能晚于 endDate' });
        }
        let list = days.filter((d) => d && d.date);
        if (startDate) list = list.filter((d) => d.date >= startDate);
        if (endDate) list = list.filter((d) => d.date <= endDate);
        // 最近在前，缺省/limit 只取有限条，绝不把整段历史塞给模型。
        list = list.sort((a, b) => (a.date < b.date ? 1 : a.date > b.date ? -1 : 1)).slice(0, limit);
        const records = list.map((d) => ({
          date: d.date,
          period: !!d.period,
          flow: d.flow || null,
          symptoms: Array.isArray(d.symptoms) ? d.symptoms.filter(Boolean) : [],
          spotting: !!d.spotting,
          bbt: d.bbt != null ? d.bbt : null,
        }));
        return JSON.stringify({ code: 'OK', records, source: 'menstrualDays', isPrediction: false, count: records.length });
      }
      ...
      case 'start_listening_session': {
        ...
        return save({ session, assistantId: session.assistantId }, 'OK', {});
      }
      ...
      case 'queue_song': {
        ...
        if (songKey(track) != null && indexOfSong(doc.player.playlist, track) >= 0) {
          return save({ queued: null, note: '已在播放列表中' }, 'OK', { added: false, note: '已在播放列表中' });
        }
        doc.player.playlist = appendToQueue(doc.player.playlist, track);
        return save({ queued: track }, 'OK', { added: true });
      }
      case 'play_song': {
        const p = doc.player || {};
        ...
        if (idx >= 0) {
          ...
          return save({ playing: true, current: idx }, 'OK', {});
        }
      ...
      // ---- 用户长期记忆（轻量结构化；AI 读写删改无需用户同意，仅尊重 enabled 开关） ----
      ...
      case 'save_memory': {
        ...
        try {
          // AI 读写删改记忆无需用户同意：直接保存为正式记忆（userConfirmed=true），不走候选确认流程
          const m = memory.upsertMemory(doc, {
      ...
      case 'save_important_conversation': {
        ...
        try {
          // AI 读写删改记忆无需用户同意：直接保存为正式记忆
          const m = memory.saveImportantConversation(doc, {
            title: args.title, summary: args.summary, keyPoints,
            relatedTopic: args.relatedTopic, userConfirmed: true,
          });
          return save({ memory: m }, 'CREATED', {});
        } catch (e) { return JSON.stringify({ code: 'FAILED', error: e.message }); }
      }
      ...
      case 'get_journal': {
        // 只读，绝不修改日志。与「新日志待读」自动触发共用 readJournalEntries，10/500/3000 上限一致。
        const { count, mode, journal } = readJournalEntries(doc, { query: args.query, mode: args.mode, limit: args.limit });
        return JSON.stringify({ code: 'OK', count, mode, matched: mode === 'relevant' ? true : undefined, journal });
      }
      case 'get_statistics': {
      ...
      case 'complete_habit': {
        ...
        return save({ habit: h, done: !alreadyDone, alreadyDone }, 'OK', { done: !alreadyDone, alreadyDone });
      }
      case 'delete_habit': {
      ...
      case 'create_ai_activity': {
        ...
        if (text.length > MAX_ACTIVITY_CONTENT_LENGTH) return JSON.stringify({ code: 'FAILED', error: '动态内容过长（最多 ' + MAX_ACTIVITY_CONTENT_LENGTH + ' 字符）' });
        ...
        // AI 自我叙述正文绝不回灌：模型只需要知道「已写入」，不需要重读自己刚写下的整段动态。
      ...
      case 'set_ai_state': {
        ...
        if (!emotion) return JSON.stringify({ code: 'FAILED', error: '缺少 emotion' });
        const reason = String(args.reason || '').trim();
        // 短窗口幂等去重：相同心情（emotion+reason）在同一会话内紧接着重复设置 → 跳过，不追加重复状态、不再审计/通知。
        // 绝不永久去重（心情切换后再回来是新的状态变化）；也绝不回滚已持久化的 state（旧状态原样保留）。
        const dup = findRecentMoodDuplicate(doc, { emotion, reason, conversationId });
        if (dup) {
          // 去重命中只回传状态码 + 原因，绝不回灌完整 state（含 emotion/reason 自我叙述）。
          return JSON.stringify({ code: 'IGNORED', reason: 'duplicate_mood' });
        }
        const state = appendAiState(doc, {
          emotion,
        ...
        return save({ state }, 'CREATED', {});
      }
      // ---- 生活记录评论（AI 对日志/健康/财务记录的评论；只评论，绝不改原始记录） ----
      case 'comment_on_record': {
        const rt = String(args.recordType || '').trim();
        if (!['journal', 'health', 'finance'].includes(rt)) {
          return JSON.stringify({ code: 'INVALID', error: 'recordType 非法（仅 journal/health/finance）' });
        }
        // 按记录类型二次校验 COMMENT 权限（与 WRITE 分离；绝不因 write 权限兜底放行）
        if (!aiCan(rt, 'comment')) return JSON.stringify({ code: 'DENIED', error: `AI 无权评论「${rt}」` });
        const rid = String(args.recordId || '').trim();
        if (!rid) return JSON.stringify({ code: 'INVALID', error: '缺少 recordId' });
        const text = String(args.comment || '').trim();
        if (!text) return JSON.stringify({ code: 'FAILED', error: '缺少 comment 正文' });

        // 查找记录（财务同时覆盖记账 expenses 与购买 purchases）；记录只读，绝不改其字段
        let record = null;
        if (rt === 'journal') record = doc.journal.find((x) => x.id === rid);
        else if (rt === 'health') record = doc.health.find((x) => x.id === rid);
        else record = doc.expenses.find((x) => x.id === rid) || doc.purchases.find((x) => x.id === rid);
        if (!record) return JSON.stringify({ code: 'NOT_FOUND', error: '记录不存在' });

        const before = (doc.ai?.comments || []).find((c) => c.recordType === rt && c.recordId === rid)?.text ?? null;
        const { action, comment } = upsertAiComment(doc, { recordType: rt, recordId: rid, text, conversationId });
        // 手动审计（entityKey=null，不走自动 diff）：记录 AI 对用户记录写评论，含 recordType/recordId
        recordAudit(doc, {
          action: 'comment', aiAction: 'comment_on_record', entityType: 'ai_comment',
          entityId: comment.id, entityLabel: text.slice(0, 40),
          before: action === 'update' ? before : null, after: text,
          actor: 'assistant', reason: 'AI 对用户记录写下评论', conversationId,
          recordType: rt, recordId: rid,
        });
        await putState(userId, doc);
        // 评论正文是 AI 自我叙述：只回传 code + recordId，不回灌 comment 全文/version 等内部字段。
        return JSON.stringify({
          code: action === 'update' ? 'UPDATED' : 'CREATED',
          recordId: rid,
        });
      }
      ...
      case 'update_conversation_cache': {
        ...
        try {
          ...
          // 对话缓存是 AI 自己写的压缩摘要：只回传 code，不回灌整份 cache。
