...
const TOOL_DEFS = [
  ...
  S('get_expenses', '读取记账记录（条目、分类、金额、时间）。可按 month=YYYY-MM 过滤。', { month: { type: 'string', description: 'YYYY-MM' } }),
  S('add_expense', '记一笔账。优先从 text 解析（如「麻辣烫 20元」「今天午饭 25块」）；也可给 title+amountCents。分类缺省时按条目自动判断。', {
    text: { type: 'string' }, title: { type: 'string' }, category: { type: 'string' }, amountCents: MONEY,
    amount: { type: 'number', description: '金额（元，备选）' }, occurredAt: DATE, occurredTime: { type: 'string' }, note: { type: 'string' },
  }),
  S('update_expense', '修改一条记账', { id: { type: 'string' }, title: { type: 'string' }, category: { type: 'string' }, amountCents: MONEY, occurredAt: DATE, note: { type: 'string' } }),
  ...
  S('get_health_records', '读取健康记录（睡眠/步数/心率/卡路里/饮水，按日期）', { days: { type: 'integer', description: '最近 N 天' } }),
