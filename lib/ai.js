...
// 同时把紧跟在 tool_result 之后的 user 文本（如工具跑完后注入的「请继续」提示）合并进同一条 user 消息，
// 避免出现连续两条 user 消息导致 Anthropic 400（roles must alternate）。
export function toAnthropicMessages(messages) {
  ...
  for (const m of messages) {
    if (m.role === 'tool') {
      ...
      const msg = toAnthropicMessage(m);
      const last = out[out.length - 1];
      if (msg.role === 'user' && last && last.role === 'user') {
        // 合并连续 user 消息：tool_result 数组后追加文本块；纯文本则拼接字符串。
        const text = String(msg.content ?? '');
        if (last._toolResults) last.content.push({ type: 'text', text });
...
// 工具已执行、但模型在本轮没给出最终自然语言回复时，追加到对话末尾的「请继续」提示。
// 强制模型产出最终 assistant text，绝不把 tool-only / 空回复当成最终回答（否则前端会出现空白气泡 + 只有 token 计数）。
// 语义锚点：工具只是内部执行过程，最终回复必须面向用户的原始消息/当前意图，绝不把工具名/结果/内部状态当成聊天主题。
const TOOL_FINAL_TEXT_NUDGE = '现在请直接面向用户自然地继续回复。优先回应用户刚才的原始消息、当前问题和真实意图；工具调用属于内部执行过程，不要把工具名称、工具结果、内部状态或刚才执行的操作当成回复主题。除非用户明确询问操作结果，否则不要专门汇报内部工具调用。';

// 兜底：8 轮循环耗尽仍无最终文字时的诚实占位（绝不返回空白、也绝不伪造「已成功」）。
const TOOL_LOOP_EMPTY_FALLBACK = '（刚刚的操作已经完成，但我没能组织好文字回复，请再和我说一次。）';
...
export async function chat({ model, messages = [], system, systemBlocks = null, temperature = 0.7, maxTokens = 2048, tools = [], callTool = null, onToolEvent = null }) {
  ...
  let convo = [...messages];
  let last = null;
  let toolRan = false; // 本轮对话是否已执行过至少一个工具（用于区分「首轮空回复」与「工具跑完却没给最终文字」）
  ...
  for (let turn = 0; turn < 8; turn++) {
    ...
    const hasToolCalls = !!(reply.toolCalls && reply.toolCalls.length > 0);
    const hasText = !!(reply.content && String(reply.content).trim());

    // 无工具能力：直接返回本轮结果（与旧行为一致）
    if (!doTools) return { ...reply, toolEvents };

    // 模型要求继续调用工具 → 执行并把结果作为 tool 消息拼回去继续下一轮。tool_call 本身绝不是最终回答。
    if (hasToolCalls) {
      toolRan = true;
      const toolResults = [];
    ...
    // 本轮没有工具调用、且有文字 → 正常的最终回复。
    if (hasText) return { ...reply, toolEvents };

    // 本轮没有工具调用、也没有文字，但之前已经执行过工具：
    // 模型把 tool-only 结果当成了终点却没生成最终文字 → 注入「请继续」提示，强制它产出最终 assistant text。
    if (toolRan) {
      convo = convo.concat([{ role: 'user', content: TOOL_FINAL_TEXT_NUDGE }]);
      continue;
    }

    // 首轮就空且无工具：保持旧行为，返回空回复（非工具场景，不在本次修复范围）。
    return { ...reply, toolEvents };
  }

  // 循环耗尽（8 轮）仍无最终文字：兜底诚实占位，绝不返回空白气泡。
  const finalContent = (last && last.content && String(last.content).trim()) ? last.content : TOOL_LOOP_EMPTY_FALLBACK;
  return { ...(last || { content: '', reasoningContent: '', usage: {} }), content: finalContent, toolEvents };
}
