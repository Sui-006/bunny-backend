...
// 读取某厂商的 key/baseUrl/protocol：优先网页配置（Supabase app_settings），回退环境变量
async function resolveProviderConfig(provider) {
  const app = await getAppSettings();
  if (provider === 'anthropic') {
    return {
      apiKey: app?.anthropic_api_key || config.anthropicApiKey,
  ...
  if (provider === 'openai') {
    return {
      apiKey: app?.openai_api_key || config.openaiApiKey,
      baseUrl: app?.openai_base_url || config.openaiBaseUrl,
    };
  }
  return {
    apiKey: app?.deepseek_api_key || config.deepseekApiKey,
...
export async function chat({ model, messages = [], system, temperature = 0.7, maxTokens = 2048 }) {
  ...
  if (provider === 'anthropic') {
    // Claude 走 OpenAI 兼容中转时，复用 OpenAI 兼容协议（很多中转站只提供 /chat/completions）
