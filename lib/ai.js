...
async function chatAnthropic({ baseUrl, apiKey, model, messages, system, temperature, maxTokens }) {
  const body = {
    ...
    messages: messages
      ...
      .map((m) => ({ role: m.role, content: (m.content || '') + (m.images && m.images.length ? `\n\n[用户发送了 ${m.images.length} 张图片，当前 Anthropic 原生协议未启用图片识别，请提示用户]` : '') })),
  };
 ...
 */
export async function chat({ model, messages = [], system, systemBlocks = null, temperature = 0.7, maxTokens = 2048, tools = [], callTool = null }) {
  if (config.mock) {
    return {
  ...
  for (let turn = 0; turn < 8; turn++) {
    if (isNativeAnthropic) {
      return chatAnthropic({ baseUrl, apiKey, model, messages: convo, system: systemBlocks ?? system, temperature, maxTokens });
    }
