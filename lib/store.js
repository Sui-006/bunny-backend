...
// 乐观并发写：仅当 user_state.version 仍等于 expectedVersion 时才写入（compare-and-set）。
// 返回 true=写入成功，false=版本已被其他操作改过（调用方应重新读取并合并重试）。
export async function saveUserStateIfVersion(userId, state, expectedVersion) {
  const expect = String(expectedVersion ?? 1);
  if (useSupabase) {
    const { data, error } = await supabase
      .from('users')
      .update({ user_state: state })
      .eq('id', userId)
      .eq('user_state->>version', expect)
      .select('id');
    if (error) throw error;
    return Array.isArray(data) && data.length > 0;
  }
  const map = memTable('users');
  const row = map.get(userId);
  if (!row) throw new Error('users 记录不存在');
  const cur = (typeof row.user_state === 'string' ? JSON.parse(row.user_state) : row.user_state) ?? {};
  if (String(cur.version ?? 1) !== expect) return false;
