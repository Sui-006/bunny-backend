...
// AI 心情历史分桶（与 public/index.html 内联 aiStateBuckets 保持一致，供 node 单测复用）：
// 今天 / 本周（今天以前，周一起算）/ 更早（本周以前）。每桶按 createdAt 倒序（最新在前）。
// 纯函数：now 传时间戳便于确定性测试；本地日历日切分（与前端 dstr/startOfWeek 同一语义，跨周边界正确）。
export function bucketAiStates(states, now = Date.now()) {
  const pad2 = (n) => (n < 10 ? '0' + n : '' + n);
  const dstr = (x) => x.getFullYear() + '-' + pad2(x.getMonth() + 1) + '-' + pad2(x.getDate());
  const nowDate = new Date(now);
  const today = dstr(nowDate);
  const ws = new Date(nowDate);
  ws.setDate(ws.getDate() - ((ws.getDay() + 6) % 7)); // 周一
  const weekStart = dstr(ws);
  const sorted = (states || []).slice().sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
  const todayItems = [], weekItems = [], earlierItems = [];
  for (const s of sorted) {
    const ds = dstr(new Date(s.createdAt));
    if (ds === today) todayItems.push(s);
    else if (ds >= weekStart && ds < today) weekItems.push(s);
...
// ============================================================
// 月度任务（与 public/index.html 内联 JS 保持一致，供 node 单测复用）
// 数据模型（挂在 monthly 计划上，绝不与 DB.tasks / 经期混用）：
//   monthlyTasks: [{ id, name, color, target, checkIns: ['YYYY-MM-DD'], createdAt, updatedAt }]
// 进度 = 有效打卡数 / target（真实数据，绝不画死 60% 视觉）。
// ============================================================

// 月度任务进度（0~100）。target 是「完成任务所需打卡次数」；未设置或 ≤0 视为 1（一次打卡即完成）。
// 打卡数超过 target 时封顶 100（绝不无限增长）。
...
// 月度任务打卡（幂等）：同一任务同一天只算一次，重复点击不新增。
// 返回 { changed, checkIns }（checkIns 为新数组，绝不原地改 task）。
export function monthlyTaskCheckin(task, date) {
  const base = (task && Array.isArray(task.checkIns)) ? task.checkIns : [];
  const d = String(date || '').trim();
  if (!d) return { changed: false, checkIns: base.slice() };
  if (base.includes(d)) return { changed: false, checkIns: base.slice() };
  return { changed: true, checkIns: [...base, d] };
}

// 某日已完成该任务？（日历格子显示颜色点用）—— 复用打卡数据，绝不另造一套「完成日期」。
export function monthlyTaskCheckedOn(task, date) {
  const d = String(date || '').trim();
  if (!d) return false;
  return (task && Array.isArray(task.checkIns)) ? task.checkIns.includes(d) : false;
}

// 按日期聚合「当天有哪些任务打了卡」（日历格子多色点用）。
// 返回 { 'YYYY-MM-DD': [taskId, ...] }，只来自真实 check-ins。
export function monthlyTaskCheckinsByDate(tasks) {
  const map = {};
  for (const t of (tasks || [])) {
    const id = String(t.id || '');
    for (const d of (Array.isArray(t.checkIns) ? t.checkIns : [])) {
...
// ============================================================
// 经期逐日记录（Apple Cycle Tracking 式，与 public/index.html 内联 JS 保持一致）
// 数据模型：menstrualDays: [{ date:'YYYY-MM-DD', period:bool, flow:'少|中|多', symptoms:[], spotting:bool, bbt:number }]
// 与 menstrualCycles（周期起止+预测）完全分离；绝不写入 Monthly Plan。
// ============================================================

// 按日期 upsert 一条经期日记录（同一天只保留一条，绝不产生重复记录）。
export function upsertMenstrualDay(days, date, patch) {
  const arr = Array.isArray(days) ? days : [];
  const d = String(date || '').trim();
  if (!d) return arr.slice();
  const out = arr.slice();
  const idx = out.findIndex((x) => x && x.date === d);
  if (idx >= 0) out[idx] = { ...out[idx], ...patch, date: d };
  else out.push({ date: d, ...patch });
  return out;
}

// 该日记录是否「空」（无经期/经量/症状/点滴出血/体温）——空则移除，避免堆积无用记录。
export function menstrualDayIsEmpty(day) {
  if (!day || typeof day !== 'object') return true;
  const symptoms = (Array.isArray(day.symptoms) ? day.symptoms : []).filter(Boolean);
  const hasBbt = day.bbt != null && String(day.bbt).trim() !== '';
  return !day.period && !day.flow && !day.spotting && !hasBbt && symptoms.length === 0;
}

// AI 动态正文是否「长到需要折叠」：前端默认折叠的阈值（纯 UI 判断，与存储/截断无关，数据库始终存全文）。
export function aiActivityIsLong(text) {
  return String(text || '').length > 200;
}
