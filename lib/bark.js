...
// Bark 请求超时（10s）：绝不让通知请求无限悬挂，超时明确报 TIMEOUT。
const BARK_TIMEOUT_MS = 10000;

// 真实发送一条 Bark 通知，返回标准化状态（绝不假装成功）：
//   { provider:'bark', status:'SUCCESS'|'FAILED'|'TIMEOUT'|'NOT_CONFIGURED', httpStatus?, errorCode?, at }
// 判定依据：HTTP 2xx 且响应体 code==200（Bark 成功返回 {code:200}）；非 JSON body 时退回以 HTTP 状态为准。
// icon 由本服务自动解析当前助手头像；AI 只决定 title/body/type，不决定 icon。
export async function sendBarkRequest(barkUrl, opts = {}) {
  const at = new Date().toISOString();
  if (!barkUrl) return { provider: 'bark', status: 'NOT_CONFIGURED', at };
  const resolvedIcon = opts.icon !== undefined ? toBarkIcon(opts.icon) : await resolveAssistantIcon();
  const url = buildBarkUrl(barkUrl, { ...opts, icon: resolvedIcon });
  // 开发日志：只记级别与 icon 是否配置，绝不打印含 device token 的完整 Bark URL。
  if (process.env.NODE_ENV !== 'production') {
    console.log(`[bark] 发送通知 type=${opts.level || 'normal'} iconConfigured=${Boolean(resolvedIcon)}`);
  }
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), opts.timeoutMs || BARK_TIMEOUT_MS);
  try {
    const res = await fetch(url, { method: 'GET', signal: ctrl.signal });
    clearTimeout(timer);
    const httpStatus = res.status;
    // Bark 成功：HTTP 2xx 且（若为 JSON）code==200；非 JSON body 时以 HTTP 状态为准。
    let bodyOk = true;
    try {
      const j = await res.json();
      if (j && typeof j === 'object' && j.code !== undefined) bodyOk = Number(j.code) === 200;
    } catch { bodyOk = res.ok; }
    if (res.ok && bodyOk) return { provider: 'bark', status: 'SUCCESS', httpStatus, at };
    console.warn('[bark] 通知发送失败 HTTP', httpStatus);
    return { provider: 'bark', status: 'FAILED', httpStatus, errorCode: 'HTTP_' + httpStatus, at };
  } catch (e) {
    clearTimeout(timer);
    if (e && (e.name === 'AbortError' || e.name === 'TimeoutError')) {
      console.warn('[bark] 通知发送超时');
      return { provider: 'bark', status: 'TIMEOUT', errorCode: 'TIMEOUT', at };
    }
    console.warn('[bark] 通知发送异常：', e.message);
    return { provider: 'bark', status: 'FAILED', errorCode: 'NETWORK_ERROR', at };
  }
}

// 兼容旧调用：返回 { sent, reason, ...真实状态 }（新代码请直接走 notification-engine）。
export async function sendBark({ barkUrl, ...opts }) {
  const r = await sendBarkRequest(barkUrl, opts);
  return { sent: r.status === 'SUCCESS', reason: r.status === 'SUCCESS' ? undefined : (r.errorCode || r.status), ...r };
}
