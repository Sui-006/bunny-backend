...
export const config = {
  ...
  // 上下文预算（token 估算；默认对齐旧 compress_threshold=60000，可用环境变量覆盖）
  contextMaxTokens: Number(process.env.CONTEXT_MAX_TOKENS) || 60000,
  contextSystemBudget: Number(process.env.CONTEXT_SYSTEM_BUDGET) || 4000,
  contextStyleBudget: Number(process.env.CONTEXT_STYLE_BUDGET) || 500,
  contextMemoryBudget: Number(process.env.CONTEXT_MEMORY_BUDGET) || 4000,
  contextSummaryBudget: Number(process.env.CONTEXT_SUMMARY_BUDGET) || 2500,
  contextDomainBudget: Number(process.env.CONTEXT_DOMAIN_BUDGET) || 2000,
  contextRecentBudget: Number(process.env.CONTEXT_RECENT_BUDGET) || 12000,
  contextCurrentMessageBudget: Number(process.env.CONTEXT_CURRENT_MESSAGE_BUDGET) || 8000,
  contextToolResultBudget: Number(process.env.CONTEXT_TOOL_RESULT_BUDGET) || 6000,
  summaryThreshold: Number(process.env.SUMMARY_THRESHOLD) || 12000,
