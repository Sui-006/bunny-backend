import { createClient } from '@supabase/supabase-js';
import { randomUUID } from 'node:crypto';
import { config } from './config.js';

// 会话级默认设置（与 supabase/schema.sql 中 settings 表列一致）
export const DEFAULT_SETTINGS = {
  system_prompt: '',
  temperature: 0.7,
  max_context_rounds: 60,
  max_context_tokens: 60000,
  compress_threshold: 60000,
  compress_keep_rounds: 10,
  max_reply_tokens: 2048,
  stream: false,
  use_memory: true,
  auto_compress: true,
};

// 主动消息 / 回复通知的默认值（app_settings 未显式设置时的兜底）。
// 后端 proactive/messages 与 GET /api/settings 共用这一份，杜绝「UI 显示开、后端读成关」的第二套设置。
export const PROACTIVE_DEFAULTS = {
  proactive_morning_enabled: true, proactive_morning_time: '08:00',
  proactive_noon_enabled: false, proactive_noon_time: '12:00',
  proactive_night_enabled: true, proactive_night_time: '22:00',
  proactive_idle_enabled: false, proactive_idle_hours: 6,
  proactive_cooldown_minutes: 30,
  reply_notify_enabled: true,
};

// 把 null/undefined 的主动消息列回填默认值（保留显式 true/false），作为单一默认来源。
export function effectiveAppSettings(app) {
  const out = { ...(app || {}) };
  for (const [k, def] of Object.entries(PROACTIVE_DEFAULTS)) {
    if (out[k] === null || out[k] === undefined) out[k] = def;
  }
  return out;
}

const useSupabase = Boolean(config.supabaseUrl && config.supabaseKey);
const supabase = useSupabase ? createClient(config.supabaseUrl, config.supabaseKey) : null;

// ---------------- 内存后端（未配置 Supabase 时用于本地调试） ----------------
const mem = {
  sessions: new Map(),
  messages: new Map(),
  memories: new Map(),
  settings: new Map(),
  appSettings: null,
  plans: new Map(),
};
const iso = () => new Date().toISOString();

// ---------------- sessions ----------------
export async function listSessions() {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('sessions')
      .select('*')
      .order('updated_at', { ascending: false });
    if (error) throw error;
    return sortSessions(data ?? []);
  }
  return sortSessions([...mem.sessions.values()]);
}

// 置顶优先，其次按最近更新倒序（pinned 列不存在时回退为仅按时间）
function sortSessions(list) {
  return list.sort((a, b) => {
    if (Boolean(a.pinned) !== Boolean(b.pinned)) return a.pinned ? -1 : 1;
    return a.updated_at < b.updated_at ? 1 : -1;
  });
}

export async function createSession(name = '新的对话', assistantId = null) {
  if (useSupabase) {
    const { data, error } = await supabase.from('sessions').insert({ name, assistant_id: assistantId ?? null }).select().single();
    if (error) throw error;
    return data;
  }
  const s = { id: randomUUID(), name, pinned: false, assistant_id: assistantId ?? null, created_at: iso(), updated_at: iso() };
  mem.sessions.set(s.id, s);
  return s;
}

export async function getSession(id) {
  if (useSupabase) {
    const { data, error } = await supabase.from('sessions').select('*').eq('id', id).maybeSingle();
    if (error) throw error;
    return data ?? null;
  }
  return mem.sessions.get(id) ?? null;
}

export async function updateSession(id, { name, pinned, assistant_id }) {
  const patch = {};
  if (name !== undefined) patch.name = name;
  if (pinned !== undefined) patch.pinned = pinned;
  if (assistant_id !== undefined) patch.assistant_id = assistant_id;
  if (useSupabase) {
    const { data, error } = await supabase.from('sessions').update(patch).eq('id', id).select().single();
    if (error) throw error;
    return data;
  }
  const s = mem.sessions.get(id);
  if (!s) throw new Error('会话不存在');
  if (name !== undefined) s.name = name;
  if (pinned !== undefined) s.pinned = pinned;
  if (assistant_id !== undefined) s.assistant_id = assistant_id;
  s.updated_at = iso();
  return s;
}

export async function deleteSession(id) {
  if (useSupabase) {
    const { error } = await supabase.from('sessions').delete().eq('id', id);
    if (error) throw error;
    return;
  }
  mem.sessions.delete(id);
  for (const map of [mem.messages, mem.memories, mem.settings]) {
    for (const [k, v] of map) if (v.session_id === id) map.delete(k);
  }
}

// 删除单条消息（用于「重新输出」）
export async function deleteMessage(id) {
  if (useSupabase) {
    const { error } = await supabase.from('messages').delete().eq('id', id);
    if (error) throw error;
    return;
  }
  mem.messages.delete(id);
}

// 更新单条消息（用于「编辑用户消息」）
export async function updateMessage(id, patch) {
  if (useSupabase) {
    const { data, error } = await supabase.from('messages').update(patch).eq('id', id).select().single();
    if (error) throw error;
    return data;
  }
  const m = mem.messages.get(id);
  if (!m) throw new Error('消息不存在');
  Object.assign(m, patch);
  return m;
}

// 标记某条消息之后的所有消息为不可见（用于「从这里重新生成」，软删除、不破坏历史）
export async function markMessagesAfterInvisible(sessionId, messageId) {
  const all = await listMessages(sessionId, { visibleOnly: false });
  const idx = all.findIndex((m) => m.id === messageId);
  if (idx < 0) return;
  const after = all.slice(idx + 1).map((m) => m.id);
  await markMessagesInvisible(after);
}

// 会话最后一条 AI 回复（用于「重新输出」）
export async function getLastAssistantMessage(sessionId) {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('messages')
      .select('*')
      .eq('session_id', sessionId)
      .eq('role', 'assistant')
      .eq('visible', true)
      .order('created_at', { ascending: false })
      .limit(1)
      .maybeSingle();
    if (error) throw error;
    return data ?? null;
  }
  const arr = [...mem.messages.values()]
    .filter((m) => m.session_id === sessionId && m.role === 'assistant' && m.visible !== false)
    .sort((a, b) => b.created_at.localeCompare(a.created_at));
  return arr[0] ?? null;
}

// ---------------- settings ----------------
export async function getSettings(sessionId) {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('settings')
      .select('*')
      .eq('session_id', sessionId)
      .maybeSingle();
    if (error) throw error;
    return data ?? null;
  }
  return mem.settings.get(sessionId) ?? null;
}

export async function saveSettings(sessionId, partial) {
  const merged = { ...DEFAULT_SETTINGS, ...partial };
  if (useSupabase) {
    const { data, error } = await supabase
      .from('settings')
      .upsert({ session_id: sessionId, ...merged }, { onConflict: 'session_id' })
      .select()
      .single();
    if (error) throw error;
    return data;
  }
  const row = { id: randomUUID(), session_id: sessionId, ...merged, updated_at: iso() };
  mem.settings.set(sessionId, row);
  return row;
}

// ---------------- 全局设置（app_settings，单行） ----------------
export async function getAppSettings() {
  if (useSupabase) {
    try {
      const { data, error } = await supabase
        .from('app_settings')
        .select('*')
        .eq('id', true)
        .maybeSingle();
      if (error) throw error;
      return data ?? null;
    } catch (e) {
      // app_settings 表尚未创建（迁移没跑）时回退环境变量，不阻断对话
      console.warn('[db] getAppSettings 失败，回退环境变量：', e.message);
      return null;
    }
  }
  return mem.appSettings ?? null;
}

export async function saveAppSettings(partial) {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('app_settings')
      .upsert({ id: true, ...partial }, { onConflict: 'id' })
      .select()
      .single();
    if (error) throw error;
    return data;
  }
  mem.appSettings = { id: true, ...(mem.appSettings || {}), ...partial, updated_at: iso() };
  return mem.appSettings;
}

// ---------------- messages ----------------
export async function listMessages(sessionId, { limit = null, visibleOnly = false } = {}) {
  if (useSupabase) {
    let q = supabase
      .from('messages')
      .select('*')
      .eq('session_id', sessionId)
      .order('created_at', { ascending: true });
    if (visibleOnly) q = q.eq('visible', true);
    if (limit != null) q = q.limit(limit);
    const { data, error } = await q;
    if (error) throw error;
    return data ?? [];
  }
  let arr = [...mem.messages.values()].filter((m) => m.session_id === sessionId);
  if (visibleOnly) arr = arr.filter((m) => m.visible !== false);
  arr.sort((a, b) => a.created_at.localeCompare(b.created_at));
  if (limit != null) arr = arr.slice(-limit);
  return arr;
}

export async function listVisibleMessages(sessionId) {
  return listMessages(sessionId, { visibleOnly: true });
}

export async function createMessage(
  sessionId,
  { role, content, reasoningContent = '', visible = true, metadata = null }
) {
  const row = {
    session_id: sessionId,
    role,
    content,
    reasoning_content: reasoningContent,
    visible,
    metadata,
    created_at: iso(),
  };
  if (useSupabase) {
    const { data, error } = await supabase.from('messages').insert(row).select().single();
    if (error) throw error;
    return data;
  }
  const full = { id: randomUUID(), ...row };
  mem.messages.set(full.id, full);
  return full;
}

export async function markMessagesInvisible(ids) {
  if (!ids || ids.length === 0) return;
  if (useSupabase) {
    const { error } = await supabase.from('messages').update({ visible: false }).in('id', ids);
    if (error) throw error;
    return;
  }
  for (const id of ids) {
    const m = mem.messages.get(id);
    if (m) m.visible = false;
  }
}

// 最近一条消息所属的会话（用于「主动发消息」确定目标会话）；无消息则退回最新会话
export async function getLatestActiveSessionId() {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('messages')
      .select('session_id')
      .order('created_at', { ascending: false })
      .limit(1)
      .maybeSingle();
    if (error) throw error;
    if (data?.session_id) return data.session_id;
    const sessions = await listSessions();
    return sessions[0]?.id ?? null;
  }
  let latest = null;
  let latestAt = '';
  for (const m of mem.messages.values()) {
    if (m.created_at > latestAt) {
      latestAt = m.created_at;
      latest = m.session_id;
    }
  }
  if (latest) return latest;
  const sessions = await listSessions();
  return sessions[0]?.id ?? null;
}

// 某会话最后一条消息的时间（用于空闲判断）；无消息返回 null
export async function getLastMessageAt(sessionId) {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('messages')
      .select('created_at')
      .eq('session_id', sessionId)
      .order('created_at', { ascending: false })
      .limit(1)
      .maybeSingle();
    if (error) throw error;
    return data?.created_at ?? null;
  }
  const arr = [...mem.messages.values()]
    .filter((m) => m.session_id === sessionId)
    .sort((a, b) => b.created_at.localeCompare(a.created_at));
  return arr[0]?.created_at ?? null;
}

// 某会话最后一条「用户」消息的时间（空闲判断只看用户发言，助手/主动消息不计入）。
// 这样晚安之后的沉默不会累加进空闲时间：只有用户当天白天开口后，空闲计时才重新开始。
export async function getLastUserMessageAt(sessionId) {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('messages')
      .select('created_at')
      .eq('session_id', sessionId)
      .eq('role', 'user')
      .order('created_at', { ascending: false })
      .limit(1)
      .maybeSingle();
    if (error) throw error;
    return data?.created_at ?? null;
  }
  const arr = [...mem.messages.values()]
    .filter((m) => m.session_id === sessionId && m.role === 'user')
    .sort((a, b) => b.created_at.localeCompare(a.created_at));
  return arr[0]?.created_at ?? null;
}

// 更新会话 updated_at（主动消息落库后，让该会话冒泡到列表顶部）
export async function touchSession(id) {
  if (useSupabase) {
    const { error } = await supabase.from('sessions').update({ updated_at: iso() }).eq('id', id);
    if (error) throw error;
    return;
  }
  const s = mem.sessions.get(id);
  if (s) s.updated_at = iso();
}

// ---------------- 对话摘要（Conversation Summary，存 sessions 表，与长期记忆严格分离） ----------------

export async function getConversationSummary(sessionId) {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('sessions')
      .select('summary, summary_version, summarized_until_message_id, summary_stale, summary_updated_at, summary_token_estimate, cache_key_points, cache_current_topic, cache_recent_decisions, cache_open_items, cache_last_message_at, cache_updated_at')
      .eq('id', sessionId)
      .maybeSingle();
    if (error) throw error;
    return data ?? null;
  }
  const s = mem.sessions.get(sessionId);
  if (!s) return null;
  return {
    summary: s.summary ?? null,
    summary_version: s.summary_version ?? 0,
    summarized_until_message_id: s.summarized_until_message_id ?? null,
    summary_stale: s.summary_stale ?? false,
    summary_updated_at: s.summary_updated_at ?? null,
    summary_token_estimate: s.summary_token_estimate ?? null,
    cache_key_points: s.cache_key_points ?? null,
    cache_current_topic: s.cache_current_topic ?? null,
    cache_recent_decisions: s.cache_recent_decisions ?? null,
    cache_open_items: s.cache_open_items ?? null,
    cache_last_message_at: s.cache_last_message_at ?? null,
    cache_updated_at: s.cache_updated_at ?? null,
  };
}

// 乐观并发写对话缓存（复用 summary_version 做 CAS）：仅当版本仍等于 expectedVersion 时写入。
// 缓存列与 summary 列共用一个版本号，避免缓存与摘要边界不一致。
export async function saveConversationCacheIfVersion(sessionId, patch, expectedVersion) {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('sessions')
      .update(patch)
      .eq('id', sessionId)
      .eq('summary_version', expectedVersion ?? 0)
      .select('id');
    if (error) throw error;
    return Array.isArray(data) && data.length > 0;
  }
  const s = mem.sessions.get(sessionId);
  if (!s) throw new Error('会话不存在');
  if ((s.summary_version ?? 0) !== expectedVersion) return false;
  Object.assign(s, patch);
  return true;
}

// 乐观并发写摘要：仅当 summary_version 仍等于 expectedVersion 时才写入（CAS）。
// 返回 true=写入成功；false=版本已被其他操作改过（调用方应放弃，绝不倒退 boundary）。
export async function saveConversationSummaryIfVersion(sessionId, patch, expectedVersion) {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('sessions')
      .update(patch)
      .eq('id', sessionId)
      .eq('summary_version', expectedVersion ?? 0)
      .select('id');
    if (error) throw error;
    return Array.isArray(data) && data.length > 0;
  }
  const s = mem.sessions.get(sessionId);
  if (!s) throw new Error('会话不存在');
  if ((s.summary_version ?? 0) !== expectedVersion) return false;
  Object.assign(s, patch);
  return true;
}

// 编辑/删除消息后：把摘要标为 stale，下次重建（绝不就地删旧摘要）
export async function touchSummaryStale(sessionId) {
  if (useSupabase) {
    const { error } = await supabase
      .from('sessions')
      .update({ summary_stale: true, summary_updated_at: iso() })
      .eq('id', sessionId);
    if (error) throw error;
    return;
  }
  const s = mem.sessions.get(sessionId);
  if (s) { s.summary_stale = true; s.summary_updated_at = iso(); }
}

export async function clearConversationSummary(sessionId) {
  if (useSupabase) {
    const { error } = await supabase
      .from('sessions')
      .update({ summary: null, summary_version: 0, summarized_until_message_id: null, summary_stale: false, summary_updated_at: null, summary_token_estimate: null })
      .eq('id', sessionId);
    if (error) throw error;
    return;
  }
  const s = mem.sessions.get(sessionId);
  if (s) {
    s.summary = null; s.summary_version = 0; s.summarized_until_message_id = null;
    s.summary_stale = false; s.summary_updated_at = null; s.summary_token_estimate = null;
  }
}

// 取「边界消息之后」的可见消息（用于增量摘要）；messageId 为空则返回全部可见消息
export async function listMessagesAfter(sessionId, messageId) {
  const all = await listMessages(sessionId, { visibleOnly: true });
  if (!messageId) return all;
  const idx = all.findIndex((m) => m.id === messageId);
  return idx < 0 ? all : all.slice(idx + 1);
}

// ---------------- memories ----------------
export async function listMemories(sessionId) {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('memories')
      .select('*')
      .eq('session_id', sessionId)
      .order('timestamp', { ascending: true });
    if (error) throw error;
    return data ?? [];
  }
  return [...mem.memories.values()]
    .filter((m) => m.session_id === sessionId)
    .sort((a, b) => a.timestamp.localeCompare(b.timestamp));
}

export async function createMemory(sessionId, summary) {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('memories')
      .insert({ session_id: sessionId, summary })
      .select()
      .single();
    if (error) throw error;
    return data;
  }
  const m = { id: randomUUID(), session_id: sessionId, summary, timestamp: iso() };
  mem.memories.set(m.id, m);
  return m;
}

// ---------------- plans（计划板块：待办 / 本月 / 年度） ----------------
export async function listPlans() {
  if (useSupabase) {
    const { data, error } = await supabase
      .from('plans')
      .select('*')
      .order('created_at', { ascending: true });
    if (error) throw error;
    return data ?? [];
  }
  return [...mem.plans.values()].sort((a, b) => a.created_at.localeCompare(b.created_at));
}

export async function createPlan({ category, content }) {
  const row = { category, content, done: false };
  if (useSupabase) {
    const { data, error } = await supabase.from('plans').insert(row).select().single();
    if (error) throw error;
    return data;
  }
  const p = { id: randomUUID(), ...row, created_at: iso() };
  mem.plans.set(p.id, p);
  return p;
}

export async function updatePlan(id, patch) {
  if (useSupabase) {
    const { data, error } = await supabase.from('plans').update(patch).eq('id', id).select().single();
    if (error) throw error;
    return data;
  }
  const p = mem.plans.get(id);
  if (!p) throw new Error('计划不存在');
  Object.assign(p, patch);
  return p;
}

export async function deletePlan(id) {
  if (useSupabase) {
    const { error } = await supabase.from('plans').delete().eq('id', id);
    if (error) throw error;
    return;
  }
  mem.plans.delete(id);
}
