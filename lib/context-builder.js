...
const RULES_PREAMBLE = [
  ...
  '工具调用属于内部执行过程；工具返回的成功、失败或查询结果只用于继续完成用户请求。收到 DENIED/FAILED/REQUIRES_PERMISSION/UNSUPPORTED 必须如实说明未成功，绝不假装已改好。最终自然语言回复应优先回应用户当前消息和真实意图；除非用户明确询问操作结果，否则不要为了汇报工具执行而偏离当前对话，确需告知完成时用简短自然的表达即可。',
