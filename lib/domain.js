...
export function defaultPlan(type) {
  ...
  return { id: randomUUID(), type, title, description: '', startDate, endDate, themeColor: meta.color, parentPlanId: null, archived: false, createdAt: Date.now(), updatedAt: Date.now(), longGoal: '', stageGoals: [], results: [], monthlyTasks: [] };
}
...
export function defaultState() {
  ...
  return {
    ...
    // 「新日志待读」一次性触发标记（服务端持有，前端全量写穿不覆盖）：写新 Journal 时置 true，
    // 下一条聊天消息处理时原子消费（置 false）。只存布尔，绝不复制日志内容。
    pendingJournalRead: false,
    ...
    ai: { memories: [], memoryCandidates: [], memorySignals: [], auditLog: [], activities: [], states: [], comments: [], styleProfile: {}, styleSignals: [] },
    ...
    // 健康隐私（经期 / 个人病历）
    menstrualCycles: [],
    // 经期逐日记录（经量/症状/点滴出血/基础体温），与 menstrualCycles 分离：cycles 管周期起止+预测，days 管每日记录
    menstrualDays: [],
    medicalRecords: [],
  };
}

const ARR_KEYS = ['tasks', 'plans', 'habits', 'health', 'notes', 'journal', 'shoppingLists', 'shoppingItems', 'notifications', 'alarms', 'purchases', 'expenses', 'menstrualCycles', 'menstrualDays', 'medicalRecords'];
...
export function repairState(s) {
  ...
  // 月度任务（打卡/进度/颜色）挂在 monthly 计划上：老计划对象补齐 monthlyTasks 数组，绝不丢已存数据
  for (const p of out.plans) if (!Array.isArray(p.monthlyTasks)) p.monthlyTasks = [];
  ...
  // AI 对生活记录（日志/健康/财务）的评论：保证数组存在，供前端「祂的评论」渲染
  if (!Array.isArray(out.ai.comments)) out.ai.comments = [];
  // 记忆设置：补齐缺省字段
...
// 「新日志待读」标记判定（纯函数）：前端全量写穿会覆盖服务端字段，故在写入前与现存文档比对。
// 传入 doc 的 journal 里出现现存文档没有的 id（新日志）→ 置 true；否则保留现存标记，绝不因无关写穿误清。
export function markPendingJournal(doc, existing) {
  const existingIds = new Set(((existing && existing.journal) || []).map((j) => j && j.id));
  const incoming = Array.isArray(doc && doc.journal) ? doc.journal : [];
  doc.pendingJournalRead = incoming.some((j) => j && j.id && !existingIds.has(j.id)) ? true : !!((existing && existing.pendingJournalRead) === true);
  return doc;
}
...
// 追加一条 AI 心情状态（情绪 + 强度 + 原因），供「今天的祂」/ 统一上下文读取。
// 纯文档变换（只改 doc.ai.states）；新状态 unshift 到最前，历史永久保留，绝不因 expiresAt 删除历史。
// expiresAt 只用于「判断一条心情是否仍为当前有效心情」（读取时过滤），历史记录本身永不裁剪。
export function appendAiState(doc, { emotion = '', intensity = 0, reason = '', triggerType = 'chat', sourceConversationId = null }) {
  if (!doc.ai || typeof doc.ai !== 'object') doc.ai = {};
  if (!Array.isArray(doc.ai.states)) doc.ai.states = [];
  const state = {
    id: randomUUID(),
    emotion: String(emotion || '').trim(),
    intensity: Math.max(0, Math.min(5, Math.round(Number(intensity) || 0))),
    reason: String(reason || '').trim(),
    triggerType: String(triggerType || 'chat'),
  ...
  doc.ai.states.unshift(state);
  return state;
}

// 短窗口幂等去重窗口：同一 AI 心情（emotion+reason）在此窗口内重复设置视为刷屏/重试，不再追加新状态。
// 取 5 分钟，与 music-companion-service 的 AI_COMMENT_COOLDOWN_MS 一致，避免随意拍一个很长的时间。
export const AI_MOOD_ACTIVITY_DEDUPE_WINDOW_MS = 5 * 60 * 1000;

// 判断 doc.ai.states 里是否已有一条「相同心情」落在去重窗口内。只比较最新一条（unshift 最新在前），
// 所以只对「紧接着的相同心情」去重：心情切到别的情绪再切回来（认真→开心→认真）中间被不同情绪隔开，不会被误去重；
// 窗口之外的相同心情也放行（代表新的真实事件）。绝不永久去重。
// 按会话（sourceConversationId）隔离：重试发生在同一会话内（conversationId 一致）才会命中；
// 不同会话的相同心情是彼此独立的事件，绝不跨会话误去重。返回命中的旧状态，否则 null。
export function findRecentMoodDuplicate(doc, { emotion, reason, conversationId = null }, now = Date.now(), windowMs = AI_MOOD_ACTIVITY_DEDUPE_WINDOW_MS) {
  const states = (doc && doc.ai && Array.isArray(doc.ai.states)) ? doc.ai.states : [];
  const normEmotion = String(emotion || '').trim();
  const normReason = String(reason || '').trim();
  const latest = states
    .filter((s) => s && (!s.expiresAt || s.expiresAt > now))
    .sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0))[0];
  if (!latest) return null;
  if (String(latest.emotion || '').trim() !== normEmotion) return null;
  if (String(latest.reason || '').trim() !== normReason) return null;
  if ((latest.sourceConversationId ?? null) !== (conversationId ?? null)) return null;
  if ((now - (latest.createdAt || 0)) >= windowMs) return null;
  return latest;
}

// 追加/更新一条「祂的评论」（AI 对日志/健康/财务记录的评论）。
// 每 (recordType, recordId) 仅保留一条当前评论：再次评论 = 覆盖更新（version+1）。
// 纯文档变换（只改 doc.ai.comments），绝不碰用户原始记录；供 comment_on_record 工具调用。
export function upsertAiComment(doc, { recordType, recordId, text, conversationId = null }) {
  if (!doc.ai || typeof doc.ai !== 'object') doc.ai = {};
  if (!Array.isArray(doc.ai.comments)) doc.ai.comments = [];
  const clean = String(text || '').trim();
  const existing = doc.ai.comments.find((c) => c.recordType === recordType && c.recordId === recordId);
  if (existing) {
    existing.text = clean;
  ...
  const comment = {
    id: randomUUID(), recordType, recordId, text: clean,
    createdAt: Date.now(), updatedAt: Date.now(),
    actor: 'assistant', conversationId: conversationId || null, version: 1,
  };
  doc.ai.comments.unshift(comment);
  return { action: 'create', comment };
}

// 月度任务进度（0~100）：checkIns.length / target，封顶 100（与前端 / frontend-logic 同一口径）
export function monthlyTaskProgress(task) {
  const checkIns = (task && Array.isArray(task.checkIns)) ? task.checkIns : [];
  const target = Math.max(1, Math.round(Number(task && task.target) || 0));
  return Math.min(100, Math.round((checkIns.length / target) * 100));
}
...
export function planProgress(plan) {
  ...
  // 月度计划：进度来自月度任务打卡（真实数据），绝不从 DB.tasks 反推
  if (plan.type === 'monthly') {
    const mt = Array.isArray(plan.monthlyTasks) ? plan.monthlyTasks : [];
    if (mt.length === 0) return 0;
    return Math.round(mt.reduce((s, t) => s + monthlyTaskProgress(t), 0) / mt.length);
  }
  return 0;
}
