...
const DOMAIN_PATTERNS = [
  ...
  // 记忆：用户明确要求记住 / 稳定偏好 / 称呼，触发记忆工具可用
  ['memory', /记住|记得|别忘了|以后都|以后叫|以后你叫|叫我|称呼|偏好|喜好|请保存|保存一下|记一下|记牢|记住这个|下次别忘/],
  // 新增领域：任务 / 计划 / 日历 / 习惯 / 购物 / 笔记 / 日志 / 统计 / 生活（宽口径聚合）
  ['tasks', /任务|待办|要做|该做|todo|日程|安排|提醒我|明天|今天|忙|事情|要做的事|完成了吗|做完/],
  ['plans', /计划|目标|规划|长期|阶段|月计划|周计划|实现|梦想|里程碑|进度|愿景/],
  ['calendar', /日历|日程|几号|周几|星期几|什么时候|哪天|排期|未来几天/],
  ['journal', /日志|日记|写日记|写日志|今天怎么样|这一天|经历|回忆|那天|心情/],
  ['notes', /笔记|记下来|便签|灵感|想法|备忘录|草稿/],
  ['habits', /习惯|打卡|坚持|每日|每天|规律|自律|早睡|早起/],
  ...
  // AI 动态（只读回看自己做过什么）：保守关键词，避免「动态规划」等误命中
  ['activity', /动态|活动记录|你之前做过|最近做了什么|之前的回应|回想一下/],
];
...
// 会触发「编辑工具注入」的领域（只读域如 life/journal/statistics/conversation 不注入编辑工具）
export const TOOL_DOMAIN_SET = new Set(['finance', 'menstrual', 'medical', 'health', 'music', 'memory', 'tasks', 'plans', 'calendar', 'habits', 'shopping', 'notes']);
