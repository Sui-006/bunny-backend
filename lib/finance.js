...
// 「今天」统一来自 lib/time.js（上海墙钟），此处仅 re-export 保持既有调用方兼容。
...
export function guessCategory(title) {
  ...
  // 取「最长命中关键词」所属分类：更具体的词（如「薯片」2 字）胜过宽泛单字（如「包」量词）
  // 避免「一包薯片」被「服饰·包」抢先命中，或「雨伞」被「衣·伞」单字命中。
  let best = null, bestLen = 0;
  for (const [cat, re] of CATEGORY_KEYWORDS) {
    const g = new RegExp(re.source, 'g');
    let m;
    while ((m = g.exec(t)) !== null) {
      if (m[0].length > bestLen) { best = cat; bestLen = m[0].length; }
    }
  }
  return best || '其他';
}
...
// ---- 自然语言解析 ----
// 从文本提取金额（分）。优先「币种符号在金额前（￥38）」，其次「元/块在金额后（38元）」，最后末尾数字。
// 返回 null 表示没有明确金额。
// 注意：不能把「数字+后面的￥」误判为金额，否则「×1 ￥38」会取到 1 而不是 38。
export function parseAmountCents(text) {
  const t = String(text || '');
  let m = t.match(/[￥¥]\s*(\d+(?:\.\d{1,2})?)/);
  if (m) return yuanToCents(m[1]);
  m = t.match(/(\d+(?:\.\d{1,2})?)\s*[元块]/);
  if (m) return yuanToCents(m[1]);
  m = t.match(/(\d+(?:\.\d{1,2})?)\s*$/);
  if (m) return yuanToCents(m[1]);
  return null;
...
export function parseExpenseText(text) {
  ...
  // 条目名：清理后仅去掉「显式分类词」本身，保留真正的条目名（不再误删命中分类关键词的条目名）
  let title = cleaned;
  if (category && category !== '其他') title = cleaned.split(/\s+/).filter((w) => w !== category).join(' ').trim();
...
// ---- 统一倒序排序（后端 / 前端 / AI 共用同一口径，绝不在某页面临时 reverse 数组）----
// 账单排序键：occurredAt（账单发生日期）倒序 → occurredTime（发生时间）倒序 → createdAt（创建时间）倒序 → id。
// 这样「最新账单最上面」「新建账单立即在顶部」「同日期按创建时间稳定排序」都成立；
// 编辑金额/分类/备注只改 updatedAt，不会改变位置；只有修改 occurredAt 才会重新排序。
export function compareExpenseDesc(a, b) {
  const da = String(a.occurredAt || ''), db = String(b.occurredAt || '');
  if (da !== db) return da < db ? 1 : -1;
  const ta = String(a.occurredTime || ''), tb = String(b.occurredTime || '');
  if (ta !== tb) return ta < tb ? 1 : -1;
  const ca = Number(a.createdAt) || 0, cb = Number(b.createdAt) || 0;
  if (ca !== cb) return cb - ca;
  return String(a.id || '') < String(b.id || '') ? -1 : String(a.id || '') > String(b.id || '') ? 1 : 0;
}
export function sortExpensesDesc(list) { return (list || []).slice().sort(compareExpenseDesc); }
export function comparePurchasesDesc(a, b) {
  const da = String(a.purchasedAt || ''), db = String(b.purchasedAt || '');
  if (da !== db) return da < db ? 1 : -1;
  const ca = Number(a.createdAt) || 0, cb = Number(b.createdAt) || 0;
  if (ca !== cb) return cb - ca;
  return String(a.id || '') < String(b.id || '') ? -1 : String(a.id || '') > String(b.id || '') ? 1 : 0;
}
export function sortPurchasesDesc(list) { return (list || []).slice().sort(comparePurchasesDesc); }
...
export function financeSummary(doc, today = todayStr()) {
  const expenses = sortExpensesDesc(doc.expenses || []);
  const purchases = sortPurchasesDesc(doc.purchases || []);

  const mk = monthKey(today);
  const ws = startOfWeek(today);
  const sum = (arr) => arr.reduce((s, e) => s + (Number(e.amountCents) || 0), 0);
  // 收入与支出并存于 expenses 数组，用 kind 区分（缺省视为支出）
  const isExpense = (e) => e.kind !== 'income';

  const monthExpenses = expenses.filter((e) => isExpense(e) && monthKey(e.occurredAt) === mk);
  const monthExpenseCents = sum(monthExpenses);
  const monthIncomeCents = sum(expenses.filter((e) => !isExpense(e) && monthKey(e.occurredAt) === mk));
  const todayExpenseCents = sum(expenses.filter((e) => isExpense(e) && e.occurredAt === today));
  const todayIncomeCents = sum(expenses.filter((e) => !isExpense(e) && e.occurredAt === today));
  const weekExpenseCents = sum(expenses.filter((e) => isExpense(e) && e.occurredAt >= ws && e.occurredAt <= today));
  ...
  return {
    ...
    recentExpenses: expenses.filter(isExpense).slice(0, 20),
    recentIncomes: expenses.filter((e) => !isExpense(e)).slice(0, 20),
