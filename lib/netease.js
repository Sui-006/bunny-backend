// 网易云音乐封装（基于 NeteaseCloudMusicApi v4，逆向接口，非官方）。
// 登录 cookie 存内存 + 持久化到 app_settings.netease_cookie（单行全局设置）。
// 匿名可搜索/取基础播放地址；登录后可取更高音质 + 用户歌单。
import Netease from 'NeteaseCloudMusicApi';
import { getAppSettings, saveAppSettings } from './db.js';

let cookie = ''; // 形如 MUSIC_U=xxx; __csrf=xxx; ...
let loaded = false;
let _authCache = { at: 0, value: null }; // 登录态探测缓存（避免高频接口反复打 login_status）

// 网易云 song_url 接口按来源 IP 做地域校验：Render（美国）出口会被判 no_source。
// 用户本人就在国内，这里把 X-Real-IP 设为国内 IP，让后端如实反映请求来源（不影响 VIP 版权，VIP 仍需登录）。
// 可用环境变量 NETEASE_REAL_IP 覆盖。
const realIP = process.env.NETEASE_REAL_IP || '223.5.5.5';

// 内存缓存：歌曲公开数据（详情/歌词）缓存数小时，热门评论缓存数分钟。
// 目的：AI 高频读取 Music Context 时不反复打网易云逆向接口，降低限流与延迟。
const CACHE_TTL = {
  detail: 6 * 60 * 60 * 1000,   // 6 小时
  lyric: 6 * 60 * 60 * 1000,    // 6 小时
  comments: 10 * 60 * 1000,     // 10 分钟
};
const _cache = new Map(); // key -> { at, value }

function cacheGet(key) {
  const e = _cache.get(key);
  if (e && Date.now() - e.at < e.ttl) return e.value;
  if (e) _cache.delete(key);
  return undefined;
}
function cacheSet(key, ttl, value) {
  _cache.set(key, { at: Date.now(), ttl, value });
  // 简单上限，避免单曲堆积拖垮内存
  if (_cache.size > 500) {
    const oldest = [..._cache.keys()].sort((a, b) => _cache.get(a).at - _cache.get(b).at).slice(0, 200);
    for (const k of oldest) _cache.delete(k);
  }
}

async function ensureLoaded() {
  if (loaded) return;
  loaded = true;
  try {
    const s = await getAppSettings();
    if (s && s.netease_cookie) cookie = String(s.netease_cookie);
  } catch (e) {
    // 表/列不存在时忽略 —— 匿名状态仍可搜索、取部分播放地址
  }
}

export function getCookie() { return cookie; }

export async function setCookie(c) {
  cookie = c ? String(c) : '';
  loaded = true;
  _authCache = { at: 0, value: null }; // 登录态变化，清空探测缓存
  if (cookie) {
    try { await saveAppSettings({ netease_cookie: cookie }); }
    catch (e) { console.warn('[netease] 持久化 cookie 失败：', e.message); }
  }
}

const withCookie = (q = {}) => (cookie ? { ...q, cookie } : q);

function toSong(s) {
  const artists = s.artists || s.ar || [];
  const album = s.album || s.al || {};
  return {
    id: s.id,
    name: s.name || '',
    artist: artists.map((a) => a.name).join(' / '),
    album: album.name || '',
    cover: (album.picUrl || '').replace(/^http:/, 'https:'),
    duration: Math.round((s.duration || s.dt || 0) / 1000),
  };
}

// 搜索单曲（type=1，cloudsearch 返回带封面 al.picUrl）
export async function search(keywords, limit = 20) {
  await ensureLoaded();
  const r = await Netease.cloudsearch(withCookie({ keywords, type: 1, limit }));
  const songs = (r.body && r.body.result && r.body.result.songs) || [];
  return songs.map(toSong);
}

const LEVEL_BR = { standard: 128000, higher: 192000, exhigh: 320000, lossless: 999000 };

// 取播放地址。区分「有可播放 URL / 无版权·VIP（SONG_NOT_PLAYABLE）/ 接口·网络失败（SONG_URL_FAILED / NETEASE_NETWORK_ERROR）」。
// 绝不在拿不到真实 URL 时假装可播放。海外部署需 realIP 伪装国内 IP。
export async function songUrl(id, level = 'exhigh') {
  await ensureLoaded();
  const br = LEVEL_BR[level] || LEVEL_BR.exhigh;
  const q = withCookie({ id: String(id), br });
  if (realIP) q.realIP = realIP;
  try {
    const r = await Netease.song_url(q);
    const d = (r.body && r.body.data && r.body.data[0]) || null;
    if (!d) return { ok: false, code: 'SONG_URL_FAILED', url: null, message: '未返回播放地址' };
    if (!d.url) return { ok: false, code: 'SONG_NOT_PLAYABLE', url: null, message: '这首歌当前无法播放（可能无版权/VIP）' };
    // 网易云返回 http:// 直链；前端是 HTTPS 部署，统一升级为 https 避免混合内容被拦
    return { ok: true, url: d.url.replace(/^http:/, 'https:'), level: d.level, size: d.size, br: d.br };
  } catch (e) {
    return { ok: false, code: 'NETEASE_NETWORK_ERROR', url: null, message: e.message };
  }
}

export async function songDetail(ids) {
  await ensureLoaded();
  const key = 'detail:' + ids.join(',');
  const hit = cacheGet(key);
  if (hit) return hit;
  const r = await Netease.song_detail(withCookie({ ids: ids.join(',') }));
  const songs = ((r.body && r.body.songs) || []).map(toSong);
  cacheSet(key, CACHE_TTL.detail, songs);
  return songs;
}

// ---- 扫码登录 ----
export async function loginQrKey() {
  await ensureLoaded();
  const r = await Netease.login_qr_key({});
  return (r.body && r.body.data && r.body.data.unikey) || '';
}

export async function loginQrCreate(key) {
  const r = await Netease.login_qr_create({ key, qrimg: true });
  const d = (r.body && r.body.data) || {};
  return { qrurl: d.qrurl || '', qrimg: d.qrimg || '' };
}

// code: 800 过期 / 801 等待扫码 / 802 已扫码待确认 / 803 授权成功
export async function loginQrCheck(key) {
  const r = await Netease.login_qr_check({ key });
  const b = r.body || {};
  if (b.code === 803 && b.cookie) await setCookie(b.cookie);
  return { code: b.code, message: b.message || '', nickname: b.nickname || '', avatarUrl: b.avatarUrl || '' };
}

// 登录态探测：可靠区分「未登录 / 已登录 / 已失效 / 网络·接口错误」。
// 依据：无 cookie=未登录；有 cookie 但 login_status 拿不到 profile（code≠200）=已失效。
// 返回统一形状 { status: 'logged_in'|'not_logged_in'|'expired'|'error', ... }。
const AUTH_CACHE_TTL = 60 * 1000; // 1 分钟，避免 AI 高频读音乐上下文时反复打接口

export async function authState(force = false) {
  if (!force && _authCache.value && Date.now() - _authCache.at < AUTH_CACHE_TTL) return _authCache.value;
  await ensureLoaded();
  if (!cookie) {
    _authCache = { at: Date.now(), value: { status: 'not_logged_in' } };
    return _authCache.value;
  }
  let value;
  try {
    const r = await Netease.login_status({ cookie });
    const p = (r.body && r.body.data && r.body.data.profile) || null;
    value = p
      ? { status: 'logged_in', userId: p.userId, nickname: p.nickname, avatarUrl: p.avatarUrl }
      : { status: 'expired' }; // 有 cookie 但拿不到 profile → 登录已失效
  } catch (e) {
    value = { status: 'error', message: e.message };
  }
  _authCache = { at: Date.now(), value };
  return value;
}

// 兼容旧调用：返回 profile（登录有效）或 null（未登录 / 已失效 / 接口错误）。
export async function loginStatus() {
  const st = await authState();
  return st.status === 'logged_in' ? { userId: st.userId, nickname: st.nickname, avatarUrl: st.avatarUrl } : null;
}

// 把 authState() 的结果映射为统一错误码（供 REST 层与测试复用，保证错误码全局一致）。
export function authErrorOf(st) {
  if (!st || st.status === 'not_logged_in') return { code: 'NETEASE_NOT_LOGGED_IN', message: '未登录网易云音乐' };
  if (st.status === 'expired') return { code: 'NETEASE_AUTH_EXPIRED', message: '网易云音乐登录已失效，请重新登录' };
  if (st.status === 'error') return { code: 'NETEASE_NETWORK_ERROR', message: '网易云接口暂时不可用，请稍后再试' };
  return null;
}

export async function logout() { await setCookie(''); }

// ---- 用户歌单（需登录） ----
export async function userPlaylists(uid) {
  await ensureLoaded();
  const r = await Netease.user_playlist(withCookie({ uid }));
  return ((r.body && r.body.playlist) || []).map((p) => ({
    id: p.id,
    name: p.name,
    cover: (p.coverImgUrl || '').replace(/^http:/, 'https:'),
    count: p.trackCount,
  }));
}

export async function playlistDetail(id) {
  await ensureLoaded();
  const r = await Netease.playlist_detail(withCookie({ id }));
  const tracks = (r.body && r.body.playlist && r.body.playlist.tracks) || [];
  return tracks.map(toSong);
}

// 歌词（原文 + 翻译），返回原始 lrc/tlyric 文本；仅公开歌词，不涉登录态
export async function lyric(id) {
  await ensureLoaded();
  const key = 'lyric:' + String(id);
  const hit = cacheGet(key);
  if (hit) return hit;
  try {
    const r = await Netease.lyric(withCookie({ id: String(id) }));
    const b = r.body || {};
    const out = { lrc: (b.lrc && b.lrc.lyric) || '', tlyric: (b.tlyric && b.tlyric.lyric) || '' };
    cacheSet(key, CACHE_TTL.lyric, out);
    return out;
  } catch (e) { return { lrc: '', tlyric: '' }; }
}

// 热门评论（公开数据，仅取少量），供 AI 了解大众评价（需求 18）
export async function hotComments(id, limit = 10) {
  await ensureLoaded();
  const key = 'comments:' + String(id) + ':' + limit;
  const hit = cacheGet(key);
  if (hit) return hit;
  try {
    const r = await Netease.comment_hot(withCookie({ id: String(id), type: 0, limit }));
    const hots = (r.body && r.body.hotComments) || [];
    const out = hots.slice(0, limit).map((c) => ({
      user: (c.user && c.user.nickname) || '',
      content: c.content || '',
      likedCount: c.likedCount || 0,
    }));
    cacheSet(key, CACHE_TTL.comments, out);
    return out;
  } catch (e) { return []; }
}

// 歌曲公开信息聚合：详情 + 歌词 + 热门评论（供 AI 读取，不含任何登录态/私有数据）
export async function songInfo(id) {
  const [detailArr, lyr, comments] = await Promise.all([
    songDetail([id]),
    lyric(id),
    hotComments(id),
  ]);
  return {
    detail: detailArr[0] || null,
    lyric: [lyr.lrc, lyr.tlyric].filter(Boolean).join('\n\n'),
    hotComments: comments,
  };
}
