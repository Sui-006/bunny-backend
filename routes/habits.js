...
router.get('/:id/streak', async (req, res, next) => {
  try {
    ...
    ok(res, { streak, longest, totalCompletions: habitTotalCompletions(h.completions) });
  } catch (e) { next(e); }
});
...
router.get('/:id/history', async (req, res, next) => {
  try {
    ...
    const h = doc.habits.find((x) => x.id === req.params.id);
    if (!h) throw new HttpError(404, 'NOT_FOUND', '习惯不存在');
    ok(res, { completions: h.completions || [], streak: habitStreak(h.completions), totalCompletions: habitTotalCompletions(h.completions) });
  } catch (e) { next(e); }
});
...
// POST /api/habits/:id/complete —— 每日打卡（幂等：同一天只记录一次，重复请求不累加、不删除）
router.post('/:id/complete', async (req, res, next) => {
  try {
    ...
    ok(res, {
      ...
      alreadyDone,
      totalCompletions: habitTotalCompletions(completions),
      currentStreak: habitStreak(completions),
      streak: habitStreak(completions),
    });
  } catch (e) { next(e); }
});
